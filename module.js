...
(function(global) {
  ...
  var version = "1.0.2";
  var isCache = true;
  var MODULE_LOADING = 1, MODULE_SUCCESS = 2;  //模块加载的状态，  loading是发起loadScript， success是define成功
  ...
  function loadModuleDep(id){
    ...
    if(dep && (len = dep.length)  > 0){
      for (var i = len - 1; i >= 0; i--) {
        var item = dep[i], len1 = item.length, moduleToLoad = item[0];
        if(len1 === 1){
          loadModule(moduleToLoad);
        }else if(len1 > 1){
          var cmbUrl = cfg.domain + '/c/=', cmbAr = [];
